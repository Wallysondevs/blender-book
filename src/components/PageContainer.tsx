...
export default function PageContainer({
  title, subtitle, difficulty, children,
}: {
  title: string;
  subtitle?: string;
  ...
  return (
    ...
    >
      <div className="mb-6 pb-6 border-b border-amber-100 dark:border-stone-800">
        {difficulty && (
          <span className={`inline-block text-xs font-semibold uppercase tracking-wide px-2 py-1 rounded mb-3 ${colors[difficulty]}`}>
            {difficulty}
          </span>
        )}
        <h1 className="text-3xl sm:text-4xl font-bold text-stone-900 dark:text-white">{title}</h1>
        {subtitle && <p className="mt-2 text-stone-600 dark:text-stone-400">{subtitle}</p>}
      </div>
